...
Usage:
 Fortran files must end in .f90 or .F90 extension.
...
def diff_cwd_working(jules_source, path, saved_umask, tmpdir):
    ...
    if diff.returncode == 0:
        ...
        print("[FAIL] The following files where changed when the "
...
def run_umdp3checker(jules_source, path):
    '''Run the umdp3 fixer script in the tmp dir copy fo the working branch.'''
    umdp3 = subprocess.Popen(jules_source + "/rose-stem/bin/umdp3_fixer.py " +
                             "$(find " + path + " -name '*.[F|f]90' | xargs)",
                             stdin=subprocess.PIPE,
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE,
                             shell=True)
