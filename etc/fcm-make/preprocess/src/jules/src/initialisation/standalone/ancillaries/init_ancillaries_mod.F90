...
SUBROUTINE init_rivers_props()
  ...
  USE jules_rivers_mod, ONLY :                                                &
       ...
       rivers_reglatlon, rivers_regrid, l_rivers, l_riv_overbank,             &
       ...
       il_river_grid, ir_land_grid
 
  USE jules_riversparm, ONLY: a_thresh

  USE rivers_utils, ONLY: rivers_get_xy_pos, rivers_earth_area,               &
                          rivers_remap_match, rivers_remap_unmatch,           &
...
! Variables passed to fill_variables_from_file
  ...
  INTEGER :: i,ip,ix,iy,l,j  ! Index variables
  INTEGER :: inext, jnext
  INTEGER :: i1, i2, ilat, ilon, step
  REAL :: reg_lat2, reg_lon2, min_lon, dlat, dlon, tiny_0
  REAL :: contribarea   ! (upstream contributing drainag area (km^2)

!Conversion from m2 to km2
  REAL, PARAMETER :: m2tokm2 = 1.0e-6
...
! before translating to the river routing grid
  IF ( l_riv_overbank ) CALL init_overbank()
...
! Initialise array values
  ...
  IF ( rivers_type == rivers_rfm ) THEN
...
! HL: Note this line is currently included for trip only to preserve rose stem
!     testing between 'xy' and 'vector' RFM approaches
...
! Checks for regular latitude/longitude grid
  IF ( rivers_reglatlon ) THEN
    ...
    tiny_0 = TINY(0.0)
    IF ( ABS(rivers_dlat) - ABS(rivers_dlon) > tiny_0 ) THEN 
      CALL log_fatal("init_rivers_props",                                     &
        ...
        DO iy=1,ny_rivers
          DO ix=1,nx_rivers
             rivers_boxareas(ix,iy) = ABS( rivers_earth_area(                 &
                                                rivers_lat2d(ix,iy) - dlat,   &
                                                rivers_lat2d(ix,iy) + dlat,   &
                                                rivers_lon2d(ix,iy) - dlon,   &
                                                rivers_lon2d(ix,iy) + dlon )) 
        END DO
      END DO
...
!-----------------------------------------------------------------------------
! Initialise river overbank parameters. 
!-----------------------------------------------------------------------------
  IF ( l_riv_overbank ) THEN
    riv_a = 7.2                     ! Andreadis et al. (2013)
    riv_b = 0.5                     ! Andreadis et al. (2013)
    riv_c = 0.27                    ! Andreadis et al. (2013)
    riv_f = 0.3                     ! Andreadis et al. (2013)
    IF ( use_rosgen ) THEN
      coef_b = 0.08                 ! Cragun (2005)
      exp_c = 0.95                  ! Cragun (2005)
      ent_ratio = 2.0               ! Rosgen (1994)
    END IF
  END IF
