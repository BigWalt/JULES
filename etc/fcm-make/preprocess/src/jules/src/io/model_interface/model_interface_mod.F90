...
!-----------------------------------------------------------------------------
! Gridbox totals of ECOSSE prognostic variables.
!-----------------------------------------------------------------------------
...
! Variable type
    VAR_TYPE_SURFACE,                                                         &
! Long name
    "Open water fraction from river inundation",                              &
! Units
...
FUNCTION extract_var(var_id) RESULT(cube)
  ...
  USE ancil_info, ONLY:                                                       &
    dim_cslayer, frac_surft, land_pts, land_index, lice_index, nsurft,        &
    soil_pts, soil_index, surft_index, surft_pts, nsoilt, frac_soilt

  USE p_s_parms, ONLY:                                                        &
    bexp_soilt, clay_soilt, sathh_soilt, satcon_soilt, smvcst_soilt,          &
    smvccl_soilt, smvcwt_soilt, hcap_soilt, hcon_soilt,                       &
    z0_surft, z0h_bare_surft, sthu_soilt, sthf_soilt,                         &
    catch_surft, cosz_ij, sthu_min_soilt, soil_ph_soilt

  USE soil_ecosse_vars_mod, ONLY:                                            &
    co2_soil_gb, n_amm_soilt, n_nit_soilt, plant_input_c_gb, plant_input_n_gb

  USE fluxes, ONLY:                                                           &
    alb_surft, fqw_surft, hf_snow_melt_gb, sub_surf_roff_gb,                  &
    surf_roff_gb, snomlt_sub_htf_gb, snow_melt_gb,                            &
    tot_tfall_gb, ecan_ij, ei_ij, esoil_ij_soilt, fqw_1_ij,                   &
  ...
  USE trif_vars_mod, ONLY:                                                    &
    ...
    litterc_pft, lit_n_pft, lit_n_t_gb, n_leach_soilt, n_fix_gb,              &
    n_fix_pft, n_gas_gb,                                                      &
    ...
    deposition_n_gb, g_burn_pft, n_loss_gb, dpm_ratio_gb, fapar_diag_pft,     &
    fao_et0, n_fertiliser_pft, n_fertiliser_gb, lit_n_ag_pft, lit_n_ag_pft,   &
    root_abandon_pft, root_abandon_gb, harvest_pft, harvest_gb,               &
    harvest_n_pft, harvest_n_gb, root_abandon_n_pft, root_abandon_n_gb,       &
    lit_n_orig_pft, frac_past_gb,                                             &
    ...
    cnsrv_nitrogen_triffid_gb, cnsrv_vegN_triffid_gb, cnsrv_soilN_triffid_gb, &
    cnsrv_n_inorg_triffid_gb, resp_s_to_atmos_gb,                             &
    npp_n, npp_n_gb, lit_c_fire_pft, burnt_carbon_dpm, lit_n_fire_pft,        &
    burnt_carbon_rpm, veg_c_fire_emission_gb, veg_c_fire_emission_pft,        &
    g_burn_gb
  ...
  USE top_pdm, ONLY:                                                          &
    drain_soilt, fch4_wetl_soilt,                                             &
    fch4_wetl_cs_soilt, fch4_wetl_npp_soilt, fch4_wetl_resps_soilt,           &
    fsat_soilt, fwetl_soilt, qbase_soilt, qbase_zw_soilt,                     &
    dun_roff_soilt, sthzw_soilt, zw_soilt
  ...
  USE fire_vars,  ONLY:                                                       &
    flammability_ft, burnt_area, burnt_area_ft,                               &
  ...
  USE data_cube_mod, ONLY:                                                    &
    data_cube, cube_from_array, cube_safe_copy,                               &
...
! being sensitive to the implementation of integer variable ids
  SELECT CASE ( get_string_identifier(var_id) )
    ...
    CASE ( 'b' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(bexp_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for b: " //               &
                       "calculation is unphysical. Use b_soilt instead")
      END IF

    CASE ( 'b_soilt' )
      cube_land = cube_from_array(bexp_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'clay' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, dim_cslayer
          workspace_levs(:,k) = soiltiles_to_gbm(clay_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:dim_cslayer))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for clay: " //            &
                       "calculation is unphysical. Use clay_soilt instead")
      END IF

    CASE ( 'clay_soilt' )
      cube_land = cube_from_array(clay_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'sathh' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(sathh_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sathh: " //           &
                       "calculation is unphysical. Use sathh_soilt instead")
      END IF

    CASE ( 'sathh_soilt' )
      cube_land = cube_from_array(sathh_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'satcon' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(satcon_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for satcon: " //          &
                       "calculation is unphysical. Use satcon_soilt instead")
      END IF

    CASE ( 'satcon_soilt' )
      cube_land = cube_from_array(satcon_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'sm_sat' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(smvcst_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sm_sat: " //          &
                       "calculation is unphysical. Use sm_sat_soilt instead")
      END IF

    CASE ( 'sm_sat_soilt' )
      cube_land = cube_from_array(smvcst_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'sm_crit' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(smvccl_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sm_crit: " //         &
                       "calculation is unphysical. Use sm_crit_soilt instead")
      END IF

    CASE ( 'sm_crit_soilt' )
      cube_land = cube_from_array(smvccl_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'sm_wilt' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(smvcwt_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sm_wilt: " //         &
                       "calculation is unphysical. Use sm_wilt_soilt instead")
      END IF

    CASE ( 'sm_wilt_soilt' )
      cube_land = cube_from_array(smvcwt_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'hcap' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(hcap_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for hcap: " //            &
                       "calculation is unphysical. Use hcap_soilt instead")
      END IF

    CASE ( 'hcap_soilt' )
      cube_land = cube_from_array(hcap_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'hcon' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(hcon_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for hcon: " //            &
                       "calculation is unphysical. Use hcon_soilt instead")
      END IF

    CASE ( 'hcon_soilt' )
      cube_land = cube_from_array(hcon_soilt)
      cube = map_from_land(cube_land)
...
!-----------------------------------------------------------------------------
    CASE ( 'soil_ph' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, dim_cslayer
          workspace_cs(:,k) = soiltiles_to_gbm(soil_ph_soilt(:,:,k))
        END DO
      ...
      ELSE
        CALL log_fatal( "extract_var",                                        &
                        ...
                        "calculation is unphysical. Use soil_ph_soilt " //    &
                        "instead" )
      END IF

    CASE ( 'soil_ph_soilt' )
      cube_land = cube_from_array(soil_ph_soilt)
      cube = map_from_land(cube_land)
...
!     a timestep.
      cube_land = cube_from_array(surftiles_to_gbm( canopy_surft ))
      cube = map_from_land(cube_land)

    CASE ( 'cs_gb' )
      IF ( nsoilt == 1) THEN
        !Case for a single soil tile
        cube_land = cube_from_array(SUM(SUM(cs_pool_soilt(:,1,:,:), 3), 2))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'depth_frozen' )
      IF ( nsoilt == 1) THEN
        ...
        layer(:) = MINLOC(ones, 2, t_soil_soilt(:,1,:) >= TM)
        ...
        DO i = 2,sm_levels
          WHERE ( layer(:) == i )
            workspace_land(:) = SUM(dzSoil(1:i-1)) + dzSoil(i) *              &
                              (TM - t_soil_soilt(:,1,i-1)) /                  &
                              (t_soil_soilt(:,1,i) - t_soil_soilt(:,1,i-1))
          END WHERE
        END DO
    ...
    CASE ( 'depth_frozen_sthf' )
      IF ( nsoilt == 1) THEN
        ...
        workspace_land(:) = 0.0
        layer(:) = MINLOC(ones, 2, sthf_soilt(:,1,:) == 0.0)
        WHERE ( layer(:) == 0 )
          workspace_land(:) = SUM(dzsoil(:)) - dzsoil(sm_levels) *            &
                (1-sthf_soilt(:,1,sm_levels)/                                 &
                (sthf_soilt(:,1,sm_levels)+sthu_soilt(:,1,sm_levels) -        &
                sthu_min_soilt(:,1,sm_levels)))
        ! The bottom layer can be below freezing temp but only partially frozen.
        END WHERE
        DO i = 2,sm_levels
          WHERE ( layer(:) == i )
            workspace_land(:) = SUM(dzSoil(1:i-1)) - dzsoil(i-1) *            &
                    (1-sthf_soilt(:,1,i-1)/(sthf_soilt(:,1,i-1)+              &
                    sthu_soilt(:,1,i-1)-sthu_min_soilt(:,1,i-1)))
          END WHERE
        END DO
        ! Map the result onto the full grid
        cube_land = cube_from_array(workspace_land)
        ...
        DO i = 1,sm_levels           ! HERE WE CAN NO LONGER IGNORE i=1.
          WHERE ( layer(:) == i )
            workspace_land(:) = SUM(dzSoil(1:i)) - dzSoil(i) *                &
                                sthf_soilt(:,1,i)/(sthf_soilt(:,1,i)+         &
                                sthu_soilt(:,1,i)-sthu_min_soilt(:,1,i))
          END WHERE
        END DO
        ! Map the result onto the full grid
        cube_land = cube_from_array(workspace_land)
    ...
    CASE ( 'drain' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(drain_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for drain: "           // &
                       "calculation is unphysical. Use drain_soilt instead")
      END IF

    CASE ( 'drain_soilt' )
      cube_land = cube_from_array(drain_soilt)
      cube = map_from_land(cube_land)
    ...
    CASE ( 'emis_gb' )
      cube_land = cube_from_array(surftiles_to_gbm(emis_surft))
      cube = map_from_land(cube_land)

    CASE ( 'fch4_wetl' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(fch4_wetl_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for fch4_wetl: "       // &
                       "Use fch4_wetl_soilt instead")
      END IF

    CASE ( 'fch4_wetl_soilt' )
      cube_land = cube_from_array(fch4_wetl_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'fch4_wetl_cs' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(fch4_wetl_cs_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for fch4_wetl_cs: "    // &
                       "Use fch4_wetl_cs_soilt instead")
      END IF

    CASE ( 'fch4_wetl_cs_soilt' )
      cube_land = cube_from_array(fch4_wetl_cs_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'fch4_wetl_npp' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(fch4_wetl_npp_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for fch4_wetl_npp: "   // &
                       "Use fch4_wetl_npp_soilt instead")
      END IF

    CASE ( 'fch4_wetl_npp_soilt' )
      cube_land = cube_from_array(fch4_wetl_npp_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'fch4_wetl_resps' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(fch4_wetl_resps_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for fch4_wetl_resps: " // &
                       "Use fch4_wetl_resps_soilt instead")
      END IF

    CASE ( 'fch4_wetl_resps_soilt' )
      cube_land = cube_from_array(fch4_wetl_resps_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'fsat' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(fsat_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for fsat: "            // &
                       "Use fsat_soilt instead")
      END IF

    CASE ( 'fsat_soilt' )
      cube_land = cube_from_array(fsat_soilt)
      cube = map_from_land(cube_land)
...
! Copy the result onto the full grid
    ...
    CASE ( 'fwetl' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(fwetl_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for fwetl: "           // &
                       "Use fwetl_soilt instead")
      END IF

    CASE ( 'fwetl_soilt' )
      cube_land = cube_from_array(fwetl_soilt)
      cube = map_from_land(cube_land)
...
! Calculate gridbox mean upwards longwave
      cube_land = cube_from_array(                                            &
        sbcon * surftiles_to_gbm(emis_surft * tstar_surft**4)                 &
      )
...
! Free work cubes
    ...
    CASE ( 'qbase' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(qbase_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for qbase: "           // &
                       "Use qbase_soilt instead")
      END IF

    CASE ( 'qbase_soilt' )
      cube_land = cube_from_array(qbase_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'qbase_zw' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(qbase_zw_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for qbase_zw: "        // &
                       "Use qbase_zw_soilt instead")
      END IF

    CASE ( 'qbase_zw_soilt' )
      cube_land = cube_from_array(qbase_zw_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'rad_net' )
      cube_land = cube_from_array(surftiles_to_gbm(radnet_surft))
      cube = map_from_land(cube_land)
    ...
    CASE ( 'resp_s_gb' )
      IF ( nsoilt == 1) THEN
        !Case for a single soil tile
        cube_land = cube_from_array(SUM(SUM(resp_s_soilt(:,1,:,:), 3), 2))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
...
! HACK: We only output the total respiration for now
      cube_land = cube_from_array(SUM(resp_s_dr_out_gb(:,:,5), 2))
      cube = map_from_land(cube_land)
    ...
    CASE ( 'soil_CN' )
      IF ( nsoilt == 1) THEN
        !Case for a single soil tile
        cube_land = cube_from_array(cs_pool_soilt(:,1,:,:) / ns_pool_gb(:,:,:))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'resp_s_diag_gb' )
      cube_land = cube_from_array(SUM(resp_s_diag_gb(:,:,5), 2))
      cube = map_from_land(cube_land)

    CASE ( 'resp_s_pot_diag_gb' )
      cube_land = cube_from_array(SUM(resp_s_pot_diag_gb(:,:,5), 2))
      cube = map_from_land(cube_land)

    CASE ( 'immob_n_gb' )
      cube_land = cube_from_array(SUM(immob_n_gb(:,:,5), 2))
      cube = map_from_land(cube_land)

    CASE ( 'immob_n_pot_gb' )
      cube_land = cube_from_array(SUM(immob_n_pot_gb(:,:,5), 2))
      cube = map_from_land(cube_land)

    CASE ( 'minl_n_gb' )
      cube_land = cube_from_array(SUM(minl_n_gb(:,:,5), 2))
      cube = map_from_land(cube_land)

    CASE ( 'minl_n_pot_gb' )
      cube_land = cube_from_array(SUM(minl_n_pot_gb(:,:,5), 2))
      cube = map_from_land(cube_land)

    CASE ( 'ns_gb' )
      cube_land = cube_from_array(SUM(SUM(ns_pool_gb(:,:,1:4),DIM=3), 2))
      cube = map_from_land(cube_land)

    CASE ( 'soil_CN_gb' )
      IF ( nsoilt == 1) THEN
        cube_land =                                                           &
          cube_from_array(SUM(SUM(cs_pool_soilt(:,1,:,1:4),DIM=3), 2) /       &
          SUM(SUM(ns_pool_gb(:,:,1:4),DIM=3), 2))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output soil_CN_gb for soil tiled runs")
    ...
    CASE ( 'sat_excess_roff' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(dun_roff_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sat_excess_roff: " // &
                       "Use sat_excess_roff_soilt instead")
      END IF

    CASE ( 'sat_excess_roff_soilt' )
      cube_land = cube_from_array(dun_roff_soilt)
      cube = map_from_land(cube_land)
        ...
        ! Add on the contribution for this layer
          DO j = 1,soil_pts
            ...
            workspace_land(i) = workspace_land(i) + RHO_WATER * dz *          &
                                MAX(0.0,                                      &
                                sthu_soilt(i,1,k) * smvcst_soilt(i,1,k)       &
                                - smvcwt_soilt(i,1,k))
          END DO
          ztop = ztop + dzsoil(k)
        END DO
      ...
      IF ( nsoilt == 1) THEN
        DO k = 1,sm_levels
          DO j = 1,soil_pts
            ...
            workspace_land(i) = workspace_land(i) + RHO_WATER * dzsoil(k) *   &
                                MAX(0.0,                                      &
                                (sthu_soilt(i,1,k) + sthf_soilt(i,1,k)) *     &
                                smvcst_soilt(i,1,k))
          END DO
        END DO
      ELSE
        !We can't take a gbm of smvcst_soilt so add up manually
        DO k = 1,sm_levels
          DO m = 1,nsoilt
            DO j = 1,soil_pts
              i = soil_index(j)
              workspace_land(i) = workspace_land(i) + (frac_soilt(i,m) *      &
                                  (RHO_WATER * dzsoil(k) *                    &
                                  MAX(0.0,                                    &
                                  (sthu_soilt(i,m,k) + sthf_soilt(i,m,k)) *   &
                                  smvcst_soilt(i,m,k))))
            END DO !j
          END DO !m
        END DO !k
...
! Only include tiles where canopy snow model is used
      cube_land = cube_from_array(surftiles_to_gbm(snow_surft, canSnowTile))
      cube = map_from_land(cube_land)

    CASE ( 'snow_depth_gb' )
      cube_land = cube_from_array(surftiles_to_gbm(snowDepth_surft))
      cube = map_from_land(cube_land)
...
! Sum frac over tiles with snow.
    ...
    CASE ( 'snow_grnd_gb' )
! Only include tiles where canopy snow model is used
      cube_land = cube_from_array(surftiles_to_gbm(snow_grnd_surft,canSnowTile))
      cube = map_from_land(cube_land)
...
! Calculate sum of sice along the snow layers dimension
      DO n = 1,nsurft
        ...
        END DO
      END DO
      cube_land = cube_from_array(surftiles_to_gbm(workspace_surft))
      cube = map_from_land(cube_land)
...
! Calculate sum of sliq along the snow layers dimension
    ...
    CASE ( 'sthzw' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(sthzw_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sthzw: "           // &
                       "calculation is unphysical. Use sthzw_soilt instead")
      END IF

    CASE ( 'sthzw_soilt' )
      cube_land = cube_from_array(sthzw_soilt)
      cube = map_from_land(cube_land)
      ...
      IF ( nsoilt == 1) THEN
        ...
        DO j = 1,soil_pts
          ...
          workspace_land(i) = SUM(dzsoil(:) * MAX(0.0,                        &
                            sthu_soilt(i,1,:) * smvcst_soilt(i,1,:)))         &
                            / SUM(dzsoil(:) * MAX(0.0, smvcst_soilt(i,1,:)))
        END DO
        ! Map the result onto the full grid
        cube_land = cube_from_array(workspace_land)
        cube = map_from_land(cube_land)
...
! Assuming emissivity=1.
      cube_land = cube_from_array(( surftiles_to_gbm(tstar_surft**4) )**0.25)
      cube = map_from_land(cube_land)

    CASE ( 'zw' )
      IF ( nsoilt == 1) THEN
        cube_land = cube_from_array(soiltiles_to_gbm(zw_soilt))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for zw: "              // &
                       "Use zw_soilt instead")
      END IF

    CASE ( 'zw_soilt' )
      cube_land = cube_from_array(zw_soilt)
      cube = map_from_land(cube_land)
    ...
    CASE ( 'root_abandon_gb' )
      cube_land = cube_from_array(root_abandon_gb)
      cube = map_from_land(cube_land)
    ...
    CASE ( 'root_abandon_n_gb' )
      cube_land = cube_from_array(root_abandon_n_gb)
      cube = map_from_land(cube_land)
    ...
    CASE ( 'cs' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(cs_pool_soilt(:,1,:,:))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'resp_s_to_atmos_gb' )
      ...
      CASE ( soil_model_rothc )
        cube_land = cube_from_array( SUM(resp_s_to_atmos_gb(:,:),2) )
      CASE ( soil_model_ecosse )
        ! Convert units from kg m-2 s-1 to kg m-2 (360 days)-1.
        cube_land = cube_from_array( co2_soil_gb(:) * secs_per_360days        &
                                     / dt_soilc )
      CASE default
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'resp_s' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(resp_s_soilt(:,1,:,:))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'cnsrv_nitrogen_triffid' )
      cube_land = cube_from_array(REAL(cnsrv_nitrogen_triffid_gb))
      cube = map_from_land(cube_land)

    CASE ( 'cnsrv_vegN_triffid' )
      cube_land = cube_from_array(REAL(cnsrv_vegN_triffid_gb))
      cube = map_from_land(cube_land)

    CASE ( 'cnsrv_soilN_triffid' )
      cube_land = cube_from_array(REAL(cnsrv_soilN_triffid_gb))
      cube = map_from_land(cube_land)

    CASE ( 'cnsrv_n_inorg_triffid' )
      cube_land = cube_from_array(REAL(cnsrv_n_inorg_triffid_gb))
      cube = map_from_land(cube_land)

    CASE ( 'lit_c_fire' )
      cube_land = cube_from_array(REAL(lit_c_fire_pft))
      cube = map_from_land(cube_land)

    CASE ( 'burnt_carbon_dpm' )
      cube_land = cube_from_array(REAL(burnt_carbon_dpm))
      cube = map_from_land(cube_land)

    CASE ( 'lit_n_fire' )
      cube_land = cube_from_array(REAL(lit_n_fire_pft))
      cube = map_from_land(cube_land)

    CASE ( 'burnt_carbon_rpm' )
      cube_land = cube_from_array(REAL(burnt_carbon_rpm))
      cube = map_from_land(cube_land)

    CASE ( 'veg_c_fire_emission_gb' )
      cube_land = cube_from_array(REAL(veg_c_fire_emission_gb))
      cube = map_from_land(cube_land)

    CASE ( 'veg_c_fire_emission_pft' )
      cube_land = cube_from_array(REAL(veg_c_fire_emission_pft))
      cube = map_from_land(cube_land)

    CASE ( 'n_inorg' )
      cube_land = cube_from_array(n_inorg_soilt_lyrs(:,1,:))
      cube = map_from_land(cube_land)

    CASE ( 'n_inorg_gb' )
      IF ( nsoilt == 1 ) THEN
        IF ( soil_bgc_model == soil_model_rothc ) THEN
          cube_land = cube_from_array( SUM( n_inorg_soilt_lyrs(:,1,:), 2 ) )
        ELSE IF (  soil_bgc_model == soil_model_ecosse ) THEN
          cube_land = cube_from_array( SUM( n_nit_soilt(:,1,:) +              &
                                            n_amm_soilt(:,1,:), 2 ) )
        END IF
        cube = map_from_land(cube_land)
      ELSE
    ...
    CASE ( 'g_burn_pft' )
      cube_land = cube_from_array(REAL(g_burn_pft))
      cube = map_from_land(cube_land)

    CASE ( 'g_burn_gb' )
      cube_land = cube_from_array(REAL(g_burn_gb))
      cube = map_from_land(cube_land)
    ...
    CASE ( 'npp_n' )
      cube_land = cube_from_array(npp_n)
      cube = map_from_land(cube_land)

    CASE ( 'npp_n_gb' )
      cube_land = cube_from_array(npp_n_gb)
      cube = map_from_land(cube_land)
    ...
    CASE ( 'n_gas_gb' )
      cube_land = cube_from_array(SUM(n_gas_gb(:,:),2))
      cube = map_from_land(cube_land)

    CASE ( 'n_leach' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(n_leach_soilt(:,1))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                "Cannot output GB mean value for n_leach when nsoilt > 1")
      END IF
...
!     Don't use the snow_mass_ij variable as that is calculated under control.
      cube_land = cube_from_array(surftiles_to_gbm(snow_grnd_surft+snow_surft))
      cube = map_from_land(cube_land)
...
!     Don't use tstar as that is calculated part-way through a timestep.
      cube_land = cube_from_array(surftiles_to_gbm( tstar_surft ))
      cube = map_from_land(cube_land)
    ...
    CASE ( 'ext' )
      ...
      ELSE
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(ext_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube      = map_from_land(cube_land)
      END IF
    ...
    CASE ( 'smcl' )
      IF ( nsoilt == 1 ) THEN
        cube_land = cube_from_array(smcl_soilt(:,1,:))
        cube      = map_from_land(cube_land)
      ELSE
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(smcl_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube      = map_from_land(cube_land)
      END IF
    ...
    CASE ( 'smcl_soilt' )
      cube_land = cube_from_array(smcl_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'soil_wet' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(sthu_soilt(:,:,k)) +         &
                                soiltiles_to_gbm(sthf_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for soil_wet: " //        &
    ...
    CASE ( 'sthf' )
      IF ( nsoilt == 1 ) THEN
        DO k=1, sm_levels
          workspace_levs(:,k) = soiltiles_to_gbm(sthf_soilt(:,:,k))
        END DO
        cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sthf: " //            &
                       "calculation is unphysical. Use sthf_soilt instead")
      END IF

    CASE ( 'sthf_soilt' )
      cube_land = cube_from_array(sthf_soilt)
      cube = map_from_land(cube_land)
    ...
    CASE ( 'sthu' )
      ...
      ELSE
        CALL log_fatal("extract_var",                                         &
                       "Cannot output GB mean value for sthu: " //            &
                       "calculation is unphysical. Use sthu_soilt instead")
      END IF

    CASE ( 'sthu_soilt' )
      cube_land = cube_from_array(sthu_soilt)
      cube = map_from_land(cube_land)

    CASE ( 't_soil' )
      DO k=1, sm_levels
        workspace_levs(:,k) = soiltiles_to_gbm(t_soil_soilt(:,:,k))
      END DO
      cube_land = cube_from_array(workspace_levs(:,1:sm_levels))
      cube = map_from_land(cube_land)

    CASE ( 't_soil_soilt' )
      cube_land = cube_from_array(t_soil_soilt)
      cube = map_from_land(cube_land)
...
!     aggregated.
    ...
    CASE ( 'croplatestharvdate' )
      cube_land = cube_from_array(latestharv_date_cpft)
      cube = map_from_land(cube_land)
    ...
    CASE ( 'frac_irrig' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(frac_irr_soilt(:,1))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'frac_irrig_soilt' )
      cube_land = cube_from_array(frac_irr_soilt)
      cube = map_from_land(cube_land)

    CASE ( 'sthu_irr' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(sthu_irr_soilt(:,1,:))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'sthu_irr_soilt' )
      cube_land = cube_from_array(sthu_irr_soilt)
      cube = map_from_land(cube_land)
...
! INFERNO variables
    CASE ( 'flammability' )
      cube_land = cube_from_array(flammability_ft)
      cube = map_from_land(cube_land)
...
!-----------------------------------------------------------------------------
    CASE ( 'c_bio' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(cs_pool_soilt(:,1,:,3))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'c_dpm' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(cs_pool_soilt(:,1,:,1))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'c_hum' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(cs_pool_soilt(:,1,:,4))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'c_rpm' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(cs_pool_soilt(:,1,:,2))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'n_bio' )
      cube_land = cube_from_array(ns_pool_gb(:,:,3))
      cube = map_from_land(cube_land)

    CASE ( 'n_dpm' )
      cube_land = cube_from_array(ns_pool_gb(:,:,1))
      cube = map_from_land(cube_land)

    CASE ( 'n_hum' )
      cube_land = cube_from_array(ns_pool_gb(:,:,4))
      cube = map_from_land(cube_land)

    CASE ( 'n_rpm' )
      cube_land = cube_from_array(ns_pool_gb(:,:,2))
      cube = map_from_land(cube_land)
...
! All are coded for a single soil tile only.
    CASE ( 'c_bio_gb' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array( SUM(cs_pool_soilt(:,1,:,3),2) )
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'c_dpm_gb' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array( SUM(cs_pool_soilt(:,1,:,1),2) )
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'c_hum_gb' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array( SUM(cs_pool_soilt(:,1,:,4),2) )
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'c_rpm_gb' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array( SUM(cs_pool_soilt(:,1,:,2),2) )
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'n_bio_gb' )
      cube_land = cube_from_array( SUM(ns_pool_gb(:,:,3),2) )
      cube = map_from_land(cube_land)

    CASE ( 'n_dpm_gb' )
      cube_land = cube_from_array( SUM(ns_pool_gb(:,:,1),2) )
      cube = map_from_land(cube_land)

    CASE ( 'n_hum_gb' )
      cube_land = cube_from_array( SUM(ns_pool_gb(:,:,4),2) )
      cube = map_from_land(cube_land)

    CASE ( 'n_rpm_gb' )
      cube_land = cube_from_array( SUM(ns_pool_gb(:,:,2),2) )
      cube = map_from_land(cube_land)
...
! Gridbox totals of derived variables for multi-pool soil models.
    CASE ( 'n_soil_gb' )
      ...
      IF (nsoilt == 1) THEN
        ! Sum organic pools.
        workspace_land(:) = SUM( SUM(ns_pool_gb(:,:,:),3), 2 ) 
        ! Add inorganic pools.
        IF ( soil_bgc_model == soil_model_rothc ) THEN
          workspace_land(:) = workspace_land(:) +                               &
                              SUM( n_inorg_soilt_lyrs(:,1,:), 2 )
        ELSE IF ( soil_bgc_model == soil_model_ecosse ) THEN
          workspace_land(:) = workspace_land(:) +                               &
                              SUM( n_amm_soilt(:,1,:)+n_nit_soilt(:,1,:), 2 )
        END IF
  !     Map the result onto the full grid.
        cube_land = cube_from_array(workspace_land)
        cube = map_from_land(cube_land)
...
! All are coded for a single soil tile only.
    CASE ( 'n_amm' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(n_amm_soilt(:,1,:))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'n_nit' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array(n_nit_soilt(:,1,:))
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
...
! Gridbox totals of ECOSSE prognostic variables.
    CASE ( 'n_amm_gb' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array( SUM( SUM(n_amm_soilt,2), 2 ) )
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
    ...
    CASE ( 'n_nit_gb' )
      IF (nsoilt == 1) THEN
        cube_land = cube_from_array( SUM( SUM(n_nit_soilt,2), 2 ) )
        cube = map_from_land(cube_land)
      ELSE
        CALL log_fatal("extract_var",                                         &
...
!   ECOSSE fluxes.
    CASE ( 'plant_input_c_gb' )
      cube_land = cube_from_array( plant_input_c_gb(:) )
      cube = map_from_land(cube_land)

    CASE ( 'plant_input_n_gb' )
      cube_land = cube_from_array( plant_input_n_gb(:) )
      cube = map_from_land(cube_land)
...
SUBROUTINE populate_var(var_id, cube, const_val)
  ...
  !Science variables
  USE ancil_info, ONLY:                                                       &
    dim_cslayer, land_pts, frac_surft, nsurft, z1_tq_ij, nsoilt
  ...
  USE c_elevate, ONLY:                                                        &
    surf_hgt_surft, z_land_land

  USE p_s_parms, ONLY:                                                        &
    albsoil_soilt, albobs_sw_gb, albobs_vis_gb,                               &
    albobs_nir_gb, bexp_soilt, sathh_soilt, satcon_soilt,                     &
    smvcst_soilt, smvccl_soilt, smvcwt_soilt, hcap_soilt, hcon_soilt,         &
    sthu_soilt, clay_soilt, soil_ph_soilt

  USE soil_ecosse_vars_mod, ONLY :                                            &
    n_amm_soilt, n_nit_soilt

  USE top_pdm, ONLY:                                                          &
    fexp_soilt, ti_mean_soilt, ti_sig_soilt, sthzw_soilt, zw_soilt
  ...
  USE trif_vars_mod, ONLY:                                                    &
    deposition_n_gb, frac_past_gb, g_burn_pft

  USE urban_param, ONLY:                                                      &
    wrr_gb, hwr_gb, hgt_gb, ztm_gb, disp_gb, albwl_gb,                        &
    albrd_gb, emisw_gb, emisr_gb

  USE prognostics, ONLY:                                                      &
    canopy_surft, cs_pool_soilt, gs_gb, snow_surft,                           &
    t_soil_soilt, tstar_surft, lai_pft, canht_pft,                            &
    ...
    n_inorg_soilt_lyrs, ns_pool_gb, frac_past_prev_gb,                        &
    tsurf_elev_surft, l_broadcast_soilt

  USE crop_vars_mod, ONLY:                                                    &
    dvi_cpft, rootc_cpft, harvc_cpft,                                         &
    reservec_cpft, croplai_cpft, cropcanht_cpft,                              &
    sow_date_cpft, tt_veg_cpft, tt_rep_cpft, latestharv_date_cpft,            &
    sthu_irr_soilt, frac_irr_all
...
! z values. albsoil_gb, albobs_sw_gb, etc have no levels anyway
!
    ...
    CASE ( 'albsoil' )  !no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, albsoil_soilt(:,1))
          albsoil_soilt(:,:) = SPREAD(albsoil_soilt(:,1), 2, nsoilt)
    ...
    CASE ( 'albsoil_soilt' ) !with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, albsoil_soilt(:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate albsoil_soilt- failure in logic")
        END IF
      ELSE
        albsoil_soilt(:,:) = const_val
      END IF
    ...
    CASE ( 'clay' )  ! With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt dim.
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, clay_soilt(:,1,1:dim_cslayer))
          clay_soilt(:,:,:) = SPREAD(clay_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Do nothing special.
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, clay_soilt(:,1,1:dim_cslayer))
    ...
    CASE ( 'clay_const_z' )  ! No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, clay_soilt(:,1,1))
           clay_soilt(:,1,1:dim_cslayer) = SPREAD(clay_soilt(:,1,1), 2,       &
                                                  dim_cslayer)
           clay_soilt(:,:,:)             = SPREAD(clay_soilt(:,1,:), 2,       &
                                                  nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
    ...
    CASE ( 'clay_soilt' )  ! With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           ! Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, clay_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate clay_soilt- failure in logic")
        END IF
      ELSE
        clay_soilt(:,:,:) = const_val
      END IF
    ...
    CASE ( 'clay_const_z_soilt' ) ! No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, clay_soilt(:,1:nsoilt,1))
           clay_soilt(:,:,:) = SPREAD(clay_soilt(:,:,1), 3, dim_cslayer)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'b' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, bexp_soilt(:,1,1:sm_levels))
          bexp_soilt(:,:,:) = SPREAD(bexp_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, bexp_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'b_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, bexp_soilt(:,1,1))
           bexp_soilt(:,1,1:sm_levels) = SPREAD(bexp_soilt(:,1,1), 2, sm_levels)
           bexp_soilt(:,:,:)           = SPREAD(bexp_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'b_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, bexp_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'b_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, bexp_soilt(:,1:nsoilt,1))
           bexp_soilt(:,:,:) = SPREAD(bexp_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'sathh' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sathh_soilt(:,1,1:sm_levels))
          sathh_soilt(:,:,:) = SPREAD(sathh_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sathh_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'sathh_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, sathh_soilt(:,1,1))
           sathh_soilt(:,1,1:sm_levels) = SPREAD(sathh_soilt(:,1,1), 2, sm_levels)
           sathh_soilt(:,:,:)           = SPREAD(sathh_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'sathh_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, sathh_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate sathh_soilt- failure in logic")
        END IF
      ELSE
        sathh_soilt(:,:,:) = const_val
      END IF
    ...
    CASE ( 'sathh_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, sathh_soilt(:,1:nsoilt,1))
           sathh_soilt(:,:,:) = SPREAD(sathh_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'satcon' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, satcon_soilt(:,1,1:sm_levels))
          satcon_soilt(:,:,:) = SPREAD(satcon_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, satcon_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'satcon_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, satcon_soilt(:,1,1))
           satcon_soilt(:,1,1:sm_levels) = SPREAD(satcon_soilt(:,1,1), 2, sm_levels)
           satcon_soilt(:,:,:)           = SPREAD(satcon_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'satcon_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, satcon_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate satcon_soilt- failure in logic")
        END IF
      ELSE
        satcon_soilt(:,:,:) = const_val
      END IF
    ...
    CASE ( 'satcon_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, satcon_soilt(:,1:nsoilt,1))
           satcon_soilt(:,:,:) = SPREAD(satcon_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'sm_sat' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, smvcst_soilt(:,1,1:sm_levels))
          smvcst_soilt(:,:,:) = SPREAD(smvcst_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, smvcst_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'sm_sat_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvcst_soilt(:,1,1))
           smvcst_soilt(:,1,1:sm_levels) = SPREAD(smvcst_soilt(:,1,1), 2, sm_levels)
           smvcst_soilt(:,:,:)           = SPREAD(smvcst_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'sm_sat_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvcst_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'sm_sat_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvcst_soilt(:,1:nsoilt,1))
           smvcst_soilt(:,:,:) = SPREAD(smvcst_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'sm_crit' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, smvccl_soilt(:,1,1:sm_levels))
          smvccl_soilt(:,:,:) = SPREAD(smvccl_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, smvccl_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'sm_crit_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvccl_soilt(:,1,1))
           smvccl_soilt(:,1,1:sm_levels) = SPREAD(smvccl_soilt(:,1,1), 2, sm_levels)
           smvccl_soilt(:,:,:)           = SPREAD(smvccl_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'sm_crit_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvccl_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'sm_crit_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvccl_soilt(:,1:nsoilt,1))
           smvccl_soilt(:,:,:) = SPREAD(smvccl_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'sm_wilt' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, smvcwt_soilt(:,1,1:sm_levels))
          smvcwt_soilt(:,:,:) = SPREAD(smvcwt_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, smvcwt_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'sm_wilt_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvcwt_soilt(:,1,1))
           smvcwt_soilt(:,1,1:sm_levels) = SPREAD(smvcwt_soilt(:,1,1), 2, sm_levels)
           smvcwt_soilt(:,:,:)           = SPREAD(smvcwt_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'sm_wilt_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvcwt_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'sm_wilt_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, smvcwt_soilt(:,1:nsoilt,1))
           smvcwt_soilt(:,:,:) = SPREAD(smvcwt_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'hcap' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, hcap_soilt(:,1,1:sm_levels))
          hcap_soilt(:,:,:) = SPREAD(hcap_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, hcap_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'hcap_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, hcap_soilt(:,1,1))
           hcap_soilt(:,1,1:sm_levels) = SPREAD(hcap_soilt(:,1,1), 2, sm_levels)
           hcap_soilt(:,:,:)           = SPREAD(hcap_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'hcap_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, hcap_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate hcap_soilt- failure in logic")
        END IF
      ELSE
        hcap_soilt(:,:,:) = const_val
      END IF
    ...
    CASE ( 'hcap_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, hcap_soilt(:,1:nsoilt,1))
           hcap_soilt(:,:,:) = SPREAD(hcap_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'hcon' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, hcon_soilt(:,1,1:sm_levels))
          hcon_soilt(:,:,:) = SPREAD(hcon_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, hcon_soilt(:,1,1:sm_levels))
    ...
    CASE ( 'hcon_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, hcon_soilt(:,1,1))
           hcon_soilt(:,1,1:sm_levels) = SPREAD(hcon_soilt(:,1,1), 2, sm_levels)
           hcon_soilt(:,:,:)           = SPREAD(hcon_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'hcon_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, hcon_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate hcon_soilt- failure in logic")
        END IF
      ELSE
        hcon_soilt(:,:,:) = const_val
      END IF
    ...
    CASE ( 'hcon_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, hcon_soilt(:,1:nsoilt,1))
           hcon_soilt(:,:,:) = SPREAD(hcon_soilt(:,:,1), 3, sm_levels)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'soil_ph' )  !With levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, soil_ph_soilt(:,1,1:dim_cslayer))
          soil_ph_soilt(:,:,:) = SPREAD(soil_ph_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, soil_ph_soilt(:,1,1:dim_cslayer))
    ...
    CASE ( 'soil_ph_const_z' ) !No levels, no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          ! Spread around soilt and layer dims
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, soil_ph_soilt(:,1,1))
           soil_ph_soilt(:,1,1:dim_cslayer) = SPREAD(soil_ph_soilt(:,1,1),&
                                                       2, dim_cslayer)
           soil_ph_soilt(:,:,:)           = SPREAD(soil_ph_soilt(:,1,:),  &
                                                     2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          ! Copy the values from the first vertical level to all others
          cube_land = map_to_land(cube)
    ...
    CASE ( 'soil_ph_soilt' ) !With levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, soil_ph_soilt(:,:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate soil_ph_soilt- failure in logic")
        END IF
      ELSE
        soil_ph_soilt(:,:,:) = const_val
      END IF
    ...
    CASE ( 'soil_ph_const_z_soilt' ) !No levels, with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Spread around layer dim
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, soil_ph_soilt(:,1:nsoilt,1))
           soil_ph_soilt(:,:,:) = SPREAD(soil_ph_soilt(:,:,1), 3,             &
                                           dim_cslayer)
        ELSE
          CALL log_fatal("populate_var",                                      &
    ...
    CASE ( 'fexp' )  !no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, fexp_soilt(:,1))
          fexp_soilt(:,:) = SPREAD(fexp_soilt(:,1), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, fexp_soilt(:,1))
        ...
        END IF
      ELSE
        fexp_soilt(:,:) = const_val
      END IF

    CASE ( 'fexp_soilt' ) !with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, fexp_soilt(:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate fexp_soilt- failure in logic")
        END IF
      ELSE
        fexp_soilt(:,:) = const_val
      END IF
    ...
    CASE ( 'ti_mean' )  !no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, ti_mean_soilt(:,1))
          ti_mean_soilt(:,:) = SPREAD(ti_mean_soilt(:,1), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, ti_mean_soilt(:,1))
        ...
        END IF
      ELSE
        ti_mean_soilt(:,:) = const_val
      END IF

    CASE ( 'ti_mean_soilt' ) !with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, ti_mean_soilt(:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate ti_mean_soilt- failure in logic")
        END IF
      ELSE
        ti_mean_soilt(:,:) = const_val
      END IF
    ...
    CASE ( 'ti_sig' )  !no tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_ancils) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, ti_sig_soilt(:,1))
          ti_sig_soilt(:,:) = SPREAD(ti_sig_soilt(:,1), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, ti_sig_soilt(:,1))
        ...
        END IF
      ELSE
        ti_sig_soilt(:,:) = const_val
      END IF

    CASE ( 'ti_sig_soilt' ) !with tiles
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. .NOT. l_broadcast_ancils) THEN
           !Do nothing special
           cube_land = map_to_land(cube)
           CALL cube_get_data(cube_land, ti_sig_soilt(:,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate ti_sig_soilt- failure in logic")
        END IF
      ELSE
        ti_sig_soilt(:,:) = const_val
      END IF
...
!-----------------------------------------------------------------------------
    ...
    CASE ( 'n_inorg' )
      IF ( nsoilt == 1 ) THEN
        IF ( PRESENT(cube) ) THEN
          cube_land = map_to_land(cube)
          CALL cube_get_data( cube_land, n_inorg_soilt_lyrs(:,1,:) )
        ELSE
          n_inorg_soilt_lyrs(:,:,:) = const_val
        END IF
      ELSE
        CALL log_fatal("populate_var",                                        &
                       "Cannot populate n_inorg when nsoilt > 1.")
      END IF
      ...
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          ...
          CALL cube_get_data(cube_land, n_inorg_soilt_lyrs(:,1,:))
          n_inorg_soilt_lyrs(:,:,:) =                                         &
            SPREAD(n_inorg_soilt_lyrs(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, n_inorg_soilt_lyrs(:,1,:))
...
!-----------------------------------------------------------------------------
    ...
    CASE ( 'cs' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, cs_pool_soilt(:,1,:,:))
          cs_pool_soilt(:,:,:,:) = SPREAD(cs_pool_soilt(:,1,:,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, cs_pool_soilt(:,1,:,:))
    ...
    CASE ( 'g_burn_pft' )
      IF ( PRESENT(cube) ) THEN
        cube_land = map_to_land(cube)
        CALL cube_get_data(cube_land, g_burn_pft)
      ELSE
        g_burn_pft(:,:) = const_val
      END IF
    ...
    CASE ( 'sthuf' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sthuf_soilt(:,1,:))
          sthuf_soilt(:,:,:) = SPREAD(sthuf_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sthuf_soilt(:,1,:))
    ...
    CASE ( 't_soil' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, t_soil_soilt(:,1,:))
          t_soil_soilt(:,:,:) = SPREAD(t_soil_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, t_soil_soilt(:,1,:))
    ...
    CASE ( 'sthu_irr' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sthu_irr_soilt(:,1,:))
          sthu_irr_soilt(:,:,:) = SPREAD(sthu_irr_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sthu_irr_soilt(:,1,:))
    ...
    CASE ( 'croplatestharvdate' )
      IF ( PRESENT(cube) ) THEN
        cube_land = map_to_land(cube)
        CALL cube_get_data(cube_land, latestharv_date_cpft)
      ELSE
        latestharv_date_cpft(:,:) = const_val
      END IF
    ...
    CASE ( 'sthzw' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sthzw_soilt(:,1))
          sthzw_soilt(:,:) = SPREAD(sthzw_soilt(:,1), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, sthzw_soilt(:,1))
    ...
    CASE ( 'zw' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, zw_soilt(:,1))
          zw_soilt(:,:) = SPREAD(zw_soilt(:,1), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, zw_soilt(:,1))
    ...
    CASE ( 'slope' )
      IF ( PRESENT(cube) ) THEN
        cube_land = map_to_land(cube)
        CALL cube_get_data(cube_land, slope_gb)
      ELSE
        slope_gb(:) = const_val
      END IF
    ...
    CASE ( 'n_amm' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, n_amm_soilt(:,1,:))
          n_amm_soilt(:,:,:) = SPREAD(n_amm_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, n_amm_soilt(:,1,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate n_amm- failure in logic")
        END IF
      ELSE
        n_amm_soilt(:,:,:) = const_val
      END IF
    ...
    CASE ( 'n_nit' )
      IF ( PRESENT(cube) ) THEN
        IF ( l_tile_soil .AND. l_broadcast_soilt) THEN
          !Spread around soilt dim
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, n_nit_soilt(:,1,:))
          n_nit_soilt(:,:,:) = SPREAD(n_nit_soilt(:,1,:), 2, nsoilt)
        ELSE IF ( .NOT. l_tile_soil ) THEN
          !Do nothing special
          cube_land = map_to_land(cube)
          CALL cube_get_data(cube_land, n_nit_soilt(:,1,:))
        ELSE
          CALL log_fatal("populate_var",                                      &
                         "Cannot populate n_nit- failure in logic")
        END IF
      ELSE
        n_nit_soilt(:,:,:) = const_val
      END IF
