...
,z0                                                                           &
             ! yet more workspace
,ratio                                                                        &
             ! workspace value for the roughness length ratio
,re_rough                                                                     &
             ! workspace value for the roughness Reynolds number
,wk1                                                                          &
             ! workspace 1
,wk2                                                                          &
             ! workspace 2
...
DO n = 1,nsurft
  DO k = 1,surft_pts(n)
    ...
    IF ( ( .NOT. l_aggregate) .OR. (l_aggregate .AND. i_aggregate_opt == 0) ) THEN
    !New code     
        ...
        !DO j = tdims%j_start,tdims%j_end
           !DO i = tdims%i_start,tdims%i_end
               ...
               !* vshr_ssi(i,j) * vshr_ssi(i,j) )f
            !This assumes that the viscosity of air is 1.34e-5 for 0 degrees celsius
            !re_rough = z0m_surft(l,n) !vshr_land(i,j) * z0m_surft(l,n)/1.34e-5
            !ratio = 2.46 * re_rough**0.25 - 2
            !ratio(n) = z0h_z0m(n)
            wk1 = 0.0000134
            wk2 = 2.46
            wk3 = 0.25
            wk4 = 2
            re_rough = z0m_surft(l,n) * vshr_land(i,j) / 0.0000134 !( z0m_surft(l,n) * vshr_land(i,j) ) / 1.34e-5
            !ratio = -0.317 + 0.565 * LOG (re_rough) + 0.183 * LOG (re_rough) * LOG (re_rough)
            PRINT *, z0m_surft
            ratio = 2.46 * re_rough**0.25 - 2
            !ratio = 1.29 * re_rough**0.25 -2  
            z0h_surft(l,n) = ratio * z0m_surft(l,n)
           !END DO
        !END DO
    !End of new code
    END IF
...
!$OMP        z0h_scm, z0h_sea, z0hsea, i_high_wind_drag, z0msea_max)
...
!$OMP        ice_fract, rib_sea, z0msea, z0hsea)
