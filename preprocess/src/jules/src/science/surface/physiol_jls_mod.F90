...
smc_irr_soilt(:,:)      = 0.0
wt_ext_irr_soilt(:,:,:) = 0.0
wt_ext_irr_type(:,:,:)  = 0.0
fsmc_irr(:,:)           = 0.0

!Compress some variables to land points only for st_stom
!$OMP PARALLEL IF(land_pts > 1) DEFAULT(NONE) PRIVATE(i, j, l, n, m)          &
!$OMP SHARED(land_pts, land_index, t_i_length, pstar_land, pstar, ipar_land,  &
!$OMP        photosynth_act_rad, q1_land, qw_1, ntype, gs_type, gs,           &
!$OMP        l_irrig_dmd, gs_irr_type, cosz_gb, cos_zenith_angle,             &
!$OMP        gsoil_irr_soilt, smvccl_soilt, gs_nvg, soil, npft, nsoilt,       &
