...
! Local constants for numerical integration
...
!$OMP&         phi_h_tmp,phi_rsl_tmp,phi_rsl_tmp_old,ff,tt,zz_uv,zz_tq,difzz, &
...
DO k = 1,surft_pts
  ...
  DO n = -nn, nn
    ...
    zz_uv = 0.5 * z_uv(i,j) * TANH(pi_2 * SINH(tt))                           &
  ...
  IF (l_rsl_scalar) THEN
    ...
    DO n = -nn, nn
      ...
      zz_tq = 0.5 * z_tq(i,j) * TANH(pi_2 * SINH(tt))                         &
...
DO k = -nn, nn
  ...
  tmp_expsh = EXP(pi_2 * SINH(tt))
  zz_uv = z_uv + zdt + tmp_expsh
  zz_tq = z_tq + zdt + tmp_expsh
  difzz = pi_2 * COSH(tt) * tmp_expsh
